...
int gui_device_joint_controller::startup() {
    for (int i=0; i<gui_element_.size(); i++) {
        if (gui_element_[i]->startup() != jcs::RET_OK) { 
            std::cout << "gui_device_joint_controller: " << gui_element_[i]->type_name_ << " startup error\n";
...
int gui_device_joint_controller::step_rt() {
    return jcs::RET_OK;
}

int gui_device_joint_controller::render() {
